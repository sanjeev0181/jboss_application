resource "aws_instance" "windows" {
  ...
  user_data = <<-EOF
    ...
    (New-Object System.Net.WebClient).DownloadFile("https://example.com/configure-jboss.ps1", "$env:TEMP/configure-jboss.ps1")
    powershell -ExecutionPolicy ByPass -File "$env:TEMP/configure-jboss.ps1"
    </powershell>
  EOF
}
